...
import { deleteEmployeeById } from 'apiSdk/employees';
...
function CompanyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface>(
    ...
    () =>
      getCompanyById(id, {
        relations: ['user', 'employee', 'customer'],
      }),
  );

  const employeeHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
  ...
  const customerHandleCreate = async () => {
    setCreateError(null);
    try {
      await createCustomer({ company_id: id, user_id: customerUserId });
              ...
              {hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/employees/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.employee?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/employees/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              ...
                              {hasAccess('employee', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    employeeHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
